...
public class NetworkManager : MonoBehaviour
{
    ...
    public string AvatarPassword = String.Empty;
    ...
    void Start()
    {
        ...
        if (ini.IsKeyExists("AvatarConfig", "AvatarPassword"))
        {
            ...
            Debug.Log("AvatarPassword: " + this.AvatarPassword);
        }
        else
        {
            ini.WriteValue("AvatarConfig", "AvatarPassword", "test1234!");
            Debug.Log("AvatarPassword: " + this.AvatarPassword);
        }
        //Close file
        ini.Close();
    ...
    private void ReceiveMessage(byte[] BufferReceiver)
    {
        ...
        else if (STypeBuffer == (byte)SendType.SENDTOSERVER)
        {
            ...
            if ((sbyte)CommandType.LOGIN == HMessageReceived.Command)
            {
                ...
                {
                    Debug.Log("UID RECEIVED is EMPTY (NOT VALID PASSWORD): " + HMessageReceived.Answer);
                    //Disconnect
                    if (serverConn != null)
                    {
                        Debug.Log("DisConnecting from: " + serverConn.EndPoint.ToString());
